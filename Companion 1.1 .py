...
def update_window():
    ...
    if Start_Flag:
        ...
        altitudes = []
        alt = START_HEIGHT
        for i in range(int((ABLE_TO_PASS - START_HEIGHT) / CLIMB_RATE)):
            altitudes.append(alt)
            alt += CLIMB_RATE
        for i in range(int((ABLE_TO_PASS - END_HEIGHT) / DESCENT_RATE)):
            altitudes.append(alt)
            alt -= DESCENT_RATE
        if elapsed_seconds < len(altitudes):
            ...
            speed_label.config(text="Time: {:02d}:{:02d} | Avg Lift: {:.1f} | Altitude: {:d} |  {:.1f}".format(
                minutes, seconds, average_lift, int(Current_Altitude), Current_Altitude - desired_altitude))
